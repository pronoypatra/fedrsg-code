...
def get_dataset(name: str, root: str = "./data"):
    ...
    if name in ("femnist", "emnist"):
        ...
        tr = datasets.EMNIST(root, split="balanced", train=True, download=True, transform=tf)
        te = datasets.EMNIST(root, split="balanced", train=False, download=True, transform=tf)
...
def _get_adult(root: str):
    ...
    df = ds.frame.dropna()
    y = (df["class"].astype(str).str.contains(">50K")).astype(np.int64).to_numpy()
    ...
    n = len(Xc); rng = np.random.default_rng(0); idx = rng.permutation(n); cut = int(0.8 * n)
    tr_i, te_i = idx[:cut], idx[cut:]
    def _ds(i):
        return TensorDataset(torch.from_numpy(Xc[i]), torch.from_numpy(y[i]))
    return _ds(tr_i), _ds(te_i), DataMeta(task="tabular", n_cls=2, in_dim=Xc.shape[1])
